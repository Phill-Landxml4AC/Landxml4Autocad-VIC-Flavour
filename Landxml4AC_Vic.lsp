;LandXML for Autocad - importer, creator and exporter of Victorina based LandXML data
...
(IF (= (setq scale (VLAX-LDATA-GET "LXML4AC" "scale" )) nil) (PROGN   (setq scale 500)  (VLAX-LDATA-PUT "LXML4AC" "scale" scale)   )  )
...
(setq autoloadff "E:\\Documents\\14 Jobs\\LXML\\Test Data\\Vic Backcaptured Plans 12_2021\\")
(SETQ autoloadlist (list "BANYULE"
			 "BASS COAST"
			 "BAW BAW"
			 "BAYSIDE"
			 "BOROONDARA"
...
(setq count 0)
  ;look for plan with leading 00 and checkdigit
(repeat (length autoloadlist)
  (IF (= FF NIL ) (SETQ FF (FINDFILE (strcat autoloadff (nth count autoloadlist) "\\" filename ".xml"))))
  (setq count (+ count 1))
  );r

  (setq count 0)
  ;just look for the plan as entered
  (repeat (length autoloadlist)
  (IF (= FF NIL ) (SETQ FF (FINDFILE (strcat autoloadff (nth count autoloadlist) "\\"textstring ".xml"))))
  (setq count (+ count 1))
  );r
...
(write-line "" outfile)
(write-line "<Application name=\"Landxml for Autocad Victorian Flavour\" version=\"1.3\" />" outfile)
...
(setq curlayer (getvar "CLAYER"))
  (SETQ LINES (SSGET  '((0 . "LINE,POINT,ARC"))))
      ...
      ;AP's appear to start getting check digits around AP60000 but it gets confusing with section number
	   ...
	   (while (OR (/= PSpos nil)
	     ...
	     (progn
	       ...
	       (setq refpos (fix(min PSpos TPpos PCpos BPpos LPpos CDpos CPpos CSpos APpos)))
