...
;Revision 1.4.4-Created internal linetype, menu and dcl file thanks to Daniel Vonks awesome code
;              -Added XARP
;              -XINO now prioritises roads boundaries first
...
(setq expertlevel (getvar "expert"));allow linetypes to load over other linetypes
(setvar "expert" 3)
...
(setvar "expert" expertlevel);reset expert level



;PREV VONK LINETYPE LOADER
...
;;-------------------------------------- SETUP BLANK MENU --------------------------------------;;;
 ...
 (setq ls1 (list '("XTR" "Traverse")		 '("XTA" "Arc Traverse")		 '("XTC" "Chainage Traverse") '("XTCO" "Chainage Traverse & offset")
       ...
       ls3 (list '("XDE" "Edit xdata manually (XDE)")		 '("XBE" "Edit bulk data (XBE)")		 '("XCD" "Check check digits")
       ...
       ls5 (list "-"
		 '("XIN" "Import XML file")		 '("XOUT" "Export XML file")		 '("XINO" "Import XML file using observation data")
		 '("XINOA" "Import XML automatically from folder using observation data")		 '("XINS" "Import import XML as simple stlye using observations")
		 '("XMO" "Export monuments to CSV")
		 "-"
		 '("XSS" "Set Drawing Scale")		 '("XRD" "Turn round on/off")		 '("XCT" "Tidy Coordinates")		 '("XMLC" "Shift Lot Centre Positions (XMLC)")
		 )
)
 ...
 (f-submenu "IMPORT EXPORT" vl3 ls5)
 ;; Add ls6 to menu - rmoved
...
;-----------------------------------------------------------------CREATE ADMIN SHEET FOR RE-ESTABLISHMENT-------------------------------
(defun c:XASR (/)
 ...
 (setq shts (getstring T "\nTownship (* for list):"))
  ...
  (setq exttitles (getstring T "\nTitle References:" ))
  (setq extaddress (getstring T "\nExtinguished Lot Addresses:" ))
  (setq shlpr (getstring T "\nSubject Plan lot and Number:" ))
  (setq shdl (getstring T "\nDepth Limitation:" ))
     (setq shtype (getstring T "\nSurveyed, Compiled or Partially Surveyed (S/C/P):" ))
  (if (or (= shtype "S")(= shtype "s")(= shtype "SURVEYED")(= shtype "Surveyed"))(setq shtype "surveyed"))
  (if (or (= shtype "C")(= shtype "c")(= shtype "COMPILED")(= shtype "Compiled"))(setq shtype "compiled"))
  (if (or (= shtype "P")(= shtype "P")(= shtype "PARTIALLY SURVEYED")(= shtype "partially Surveyed"))(setq shtype "partially surveyed"))
  (setq shpurpose1 "Section 22" )
  (setq shpurpose2 "Plan of Subdivision" )
  (setq shhop "Subdivision Act 1988" )
...
(setq /pos 0)
  ...
  ;changed for BricsCAD
    (SETVAR "clayer" "Admin Sheet")
 (COMMAND "._INSERT" "ADMINSHEET_RE" "_S" "1" tbinsertpoint "0" shname shlga shparish shfirm shref shts shcs shca shcp exttitles "" extaddress "" shlpr shdl shtype shpurpose2 shhop shsurveyor shregnumber shdos shjur shformat datum hdatum zone shlgacode shparishcode shpurpose1)
   	  (setvar "clayer" prevlayer)
  );DEFUN
...
(if (/= (setq spcpos2 (vl-string-position 32 east 1)) nil)(progn
							    ...
							    (setq east (substr east 1  spcpos2 ));if height exists
								)
  (setq height "0"));else
(setq east (atof east))
...
(linereader)
  ...
  (if (= importcgpoint "fromobs")
    (progn
      ...
      (princ "\nReading parcels for Roadlines")
;READ PARCELS FOR ROAD LINES
     ;linefeed to parcels

    (setq roadlines nil);emtpy roadlines list
...
(if (= pclclass "Road")(progn
			 

			 (linereader);parcel centrepoint
...
(setq lp2 (substr linetext (+ stringpos 9) (-(- wwpos 1)(+ stringpos 7))))
							 ...
							 (setq roadlines (append roadlines (list (strcat lp1 "-" lp2))(list (strcat lp2 "-"lp1))))
...
(setq cp2 (substr linetext (+ stringpos 9) (-(- wwpos 1)(+ stringpos 7))))
 ...
 (setq roadlines (append roadlines (list (strcat cp1 "-" cp2))(list (strcat cp2 "-"cp1))))
 ...
 (if (/= (vl-string-search "<IrregularLine" linetext ) nil)(progn
							     (while (=(vl-string-search "<IrregularLine/>" linetext ) nil)
							       (linereader)
							       )))
			 ...
			 ;linefeed to next parcel
			  (while (and (= (vl-string-search "</Parcels>" linetext ) nil)(= (vl-string-search "<Parcel " linetext ) nil)) ( progn
  (linereader)
  ));linereader
  ...
  (progn
  ;if not road linefeed to next parcel
 ...
 ;----------------------------------------------------------------
      (princ "\nReading observations for Obsimporter")
    (obsimporter)
    ));if fromobs
...
;calc arc internal angle
  ...
  ;(SETQ raybearing (+ (* -1 raybearing) (* 0.5 PI)))
	    ...
	    (setq curvecen (polar p1 raybearing  radius))
		       

		       ));if mast is not negative
...
;calc arc internal angle
		...
		(IF (< UNSQRMAST 0) (PROGN ;if chord is longer than diameter fit a semi circle
				      (PRINC (strcat "\nError - Chord length longer than arc diameter from " cp1 " to "cp2 "using semi circle"))
				      (setq curvecen (polar p1 (angle p1 p2) (/ (distance p1 p2) 2)))
				      )
		  (progn;else
                (SETQ MAST (SQRT UNSQRMAST ))
...
(IF (/= (setq adminsheet (ssget "_X" '((0 . "INSERT") (2 . "ADMINSHEET,ADMINSHEET_RE")))) nil)(progn
		(SETQ EN (CDR(ASSOC -1 (ENTGET (SSNAME ADMINSHEET 0)))))								
...
(setq wwpos (vl-string-position 34 ocmain (+ stringpos 14)))(setq smalladdress (substr ocmain (+ stringpos 15) (-(- wwpos 1)(+ stringpos 13)))))(setq smalladdress ""))
 ...
 (setq outfile (getfiled "Output File" "" "XML" 1))
  ...
  (if (= (substr time1 7 2) "") (setq time1 (strcat (substr time1 1 6 ) "00")))
    ...
    (setq hdatum (nth 22 attlist))
    (if (/= (setq stringpos (vl-string-search "~" hdatum )) nil)(progn
     
                      (setq datumdesc (strcat " desc=\""(substr hdatum (+ stringpos 2) 1000) "\" "))
                      (setq hdatum (substr hdatum 1 stringpos))
		      )(setq datumdesc "")
      )
  (princ "\nWriting File Header")
...
(setq &pos 0)
	    ...
	    (setq p1cgid (nth (+ 1 (vl-position (strcat "IS-" setupid) islist)) islist))
	    (setq p2cgid (nth (+ 1 (vl-position (strcat "IS-" targetid) islist)) islist))
	    (if (or (/= nil (vl-position (strcat p1cgid "-" p2cgid) roadlines))(/= nil (vl-position (strcat p2cgid "-" p1cgid) roadlines)))(setq rolayer "road"))
	   
(if (/= dist "")(progn

		   (if (= rolayer "road")(progn
...
(if (> (length normalobs) 0)(progn
  ...
  (setq travpoints (remove_nth travpoints 0))
    ...
    (progn ;if no traverse or road try sideshot
...
;if no road check for normal line from point
(if (and (/= obsdone "Y") (setq spremlist (member sp normalpoints)))
  (progn
...
(PRINC "\n Select Lines to add Refplan to:")
  ...
  (setq bdyline (ssget  '((0 . "LINE,ARC,POINT")(8 . "Boundary,Boundary Extinguished,Boundary Wall Exterior,Boundary Wall Interior,Boundary Wall Median,Boundary Wall Other,TRaverse,Sideshot,Easement,Irregular Right Lines,Monument,PM" ))))

    (setq csf (getstring "\nRef Plan(or enter to select admin sheet):"))

  (if (= csf "")
    (progn
    (setq adminsheet (ssget  '((0 . "INSERT") (2 . "ADMINSHEET"))))
...
(command "-layer" "r" "Adjoining Boundary" "Non SCFF Adjoining Boundary"
 ...
 "r" "Boundary" "Non SCFF Boundary" 
 "r" "Boundary Extinguished" "Non SCFF Boundary Extinguished" 
 "r" "Boundary Wall Exterior" "LINE-BUILDING-BOUNDARY-EXTERIOR" 
 "r" "Boundary Wall Interior" "LINE-BUILDING-BOUNDARY-INTERIOR" 
 "r" "Boundary Wall Median" "LINE-BUILDING-BOUNDARY-MEDIAN" 
 "r" "Boundary Wall Other" "LINE-BUILDING-BOUNDARY-OTHER"
 "r" "CG Points" "Non SCFF CG Points"
 "r" "Datum Points" "Non SCFF Datum Points"
 "r" "Drafting" "Non SCFF Drafting"
 "r" "Drafting AFR" "Non SCFF Drafting AFR"
 "r" "Easement" "Non SCFF Easement"
 "r" "Irregular Boundary" "LINE-NATURAL-BOUNDARY"
 "r" "Irregular Right Lines" "Non SCFF Irregular Right Lines"
 "r" "Lot Definitions" "Non SCFF Lot Definitions"
 "r" "Miscloses" "Non SCFF Miscloses"
 "r" "Monument" "POINT-REFERENCE"
 "r" "Occupation Building Return" "LINE-BUILDING-RETURN"
 "r" "Occupation Centreline" "Non SCFF Occupation Centreline"
 "r" "Occupation Fences" "Non SCFF Occupation Fences"
 "r" "Occupation Gate" "Non SCFF Occupation Gate"
 "r" "Occupation Hedge" "Non SCFF Occupation Hedge"
 "r" "Occupation Kerbs" "Non SCFF Occupation Kerbs"
 "r" "Occupation Not Fenced" "Non SCFF Occupation Not Fenced"
 "r" "Occupation Other" "Non SCFF Occupation Other"
 "r" "Occupation Railway" "Non SCFF Occupation Railway"
 "r" "Occupation Rockwall" "Non SCFF Occupation Rockwall"
 "r" "Occupation Timber Walls" "Non SCFF Occupation Timber Walls"
 "r" "Occupation Walls" "Non SCFF Occupation Walls"
 "r" "Occupations" "LINE-OFFSET"
 "r" "PM" "Non SCFF PM"
 "r" "Sideshot" "LINE-RADIATION"
 "r" "Topo" "LINE-CONNECTION"
 "r" "Traverse" "LINE-TRAVERSE" "")


  

(IF (/= (setq lots (ssget "_X" '((0 . "LWPOLYLINE") (8 . "Non SCFF Lot Definitions,Non SCFF Adjoining Boundary")))) nil)(progn
...
;seperate centrepoint coord out.
     ...
     (setq !pos1 (vl-string-position 33 xdatai 0))
                      (setq lotc (substr xdatai (+ !pos1 2) 200))
    (setq spcpos (vl-string-position 32 lotc 0))
    (setq cnorth (substr lotc 1 spcpos))
    (setq ceast (substr lotc (+ spcpos 2)))
    (setq lotccord (strcat ceast "," cnorth))
...
;-------Lots--------------
    ...
    (setq newlayer nil textlayer nil)
     (if (and (= pclclass "Lot" )(= pclstate "affected"))(setq newlayer "LOT-AFFECTED" textlayer "TEXT-LOT-ID"))
     (if (and (= pclclass "Lot" )(= pclstate "extinguished"))(setq newlayer "LOT-CANCELLED" textlayer "TEXT-LOT-ID"))
     (if (and (= pclclass "Lot" )(= pclstate "created"))(setq newlayer "LOT-CREATED" textlayer "TEXT-LOT-ID"))
     (if (and (= pclclass "Lot" )(= pclstate "encroached"))(setq newlayer "LOT-ENCROACHED" textlayer "TEXT-LOT-ID"))
     (if (and (= pclclass "Lot" )(= pclstate "existing"))(setq newlayer "LOT-EXISTING" textlayer "TEXT-LOT-ID"))

     (if (and (= pclclass "Common Property" )(= pclstate "affected"))(setq newlayer "COMMON-PROPERTY-AFFECTED" textlayer "TEXT-COMMON-PROPERTY-ID"))
     (if (and (= pclclass "Common Property" )(= pclstate "extinguished"))(setq newlayer "COMMON-PROPERTY-CANCELLED" textlayer "TEXT-COMMON-PROPERTY-ID"))
     (if (and (= pclclass "Common Property" )(= pclstate "created"))(setq newlayer "COMMON-PROPERTY-CREATED" textlayer "TEXT-COMMON-PROPERTY-ID"))
     (if (and (= pclclass "Common Property" )(= pclstate "encroached"))(setq newlayer "COMMON-PROPERTY-ENCROACHED" textlayer "TEXT-COMMON-PROPERTY-ID"))
     (if (and (= pclclass "Common Property" )(= pclstate "existing"))(setq newlayer "COMMON-PROPERTY-EXISTING" textlayer "TEXT-COMMON-PROPERTY-ID"))

     (if (and (= pclclass "Crown Allotment" )(= pclstate "affected"))(setq newlayer "CROWN-ALLOTMENT-AFFECTED" textlayer "TEXT-CROWN-ALLOTMENT-ID"))
     (if (and (= pclclass "Crown Allotment" )(= pclstate "extinguished"))(setq newlayer "CROWN-ALLOTMENT-CANCELLED" textlayer "TEXT-CROWN-ALLOTMENT-ID"))
     (if (and (= pclclass "Crown Allotment" )(= pclstate "created"))(setq newlayer "CROWN-ALLOTMENT-CREATED" textlayer "TEXT-CROWN-ALLOTMENT-ID"))
     (if (and (= pclclass "Crown Allotment" )(= pclstate "encroached"))(setq newlayer "CROWN-ALLOTMENT-ENCROACHED" textlayer "TEXT-CROWN-ALLOTMENT-ID"))
     (if (and (= pclclass "Crown Allotment" )(= pclstate "existing"))(setq newlayer "CROWN-ALLOTMENT-EXISTING" textlayer "TEXT-CROWN-ALLOTMENT-ID"))

     (if (and (= pclclass "Crown Portion" )(= pclstate "affected"))(setq newlayer "CROWN-PORTION-AFFECTED" textlayer "TEXT-CROWN-PORTION-ID"))
     (if (and (= pclclass "Crown Portion" )(= pclstate "extinguished"))(setq newlayer "CROWN-PORTION-CANCELLED" textlayer "TEXT-CROWN-PORTION-ID"))
     (if (and (= pclclass "Crown Portion" )(= pclstate "created"))(setq newlayer "CROWN-PORTION-CREATED" textlayer "TEXT-CROWN-PORTION-ID"))
     (if (and (= pclclass "Crown Portion" )(= pclstate "encroached"))(setq newlayer "CROWN-PORTION-ENCROACHED" textlayer "TEXT-CROWN-PORTION-ID"))
     (if (and (= pclclass "Crown Portion" )(= pclstate "existing"))(setq newlayer "CROWN-PORTION-EXISTING" textlayer "TEXT-CROWN-PORTION-ID"))

     (if (and (= pclclass "Reserve" )(= pclstate "affected"))(setq newlayer "RESERVE-AFFECTED" textlayer "TEXT-RESERVE-ID"))
     (if (and (= pclclass "Reserve" )(= pclstate "extinguished"))(setq newlayer "RESERVE-CANCELLED" textlayer "TEXT-RESERVE-ID"))
     (if (and (= pclclass "Reserve" )(= pclstate "created"))(setq newlayer "RESERVE-CREATED" textlayer "TEXT-RESERVE-ID"))
     (if (and (= pclclass "Reserve" )(= pclstate "encroached"))(setq newlayer "RESERVE-ENCROACHED" textlayer "TEXT-RESERVE-ID"))
     (if (and (= pclclass "Reserve" )(= pclstate "existing"))(setq newlayer "RESERVE-EXISTING" textlayer "TEXT-RESERVE-ID"))

     (if (and (= pclclass "Restriction" )(= pclstate "affected"))(setq newlayer "RESTRICTION-AFFECTED" textlayer "TEXT-RESTRICTION-ID"))
     (if (and (= pclclass "Restriction" )(= pclstate "extinguished"))(setq newlayer "RESTRICTION-CANCELLED" textlayer "TEXT-RESTRICTION-ID"))
     (if (and (= pclclass "Restriction" )(= pclstate "created"))(setq newlayer "RESTRICTION-CREATED" textlayer "TEXT-RESTRICTION-ID"))
     (if (and (= pclclass "Restriction" )(= pclstate "encroached"))(setq newlayer "RESTRICTION-ENCROACHED" textlayer "TEXT-RESTRICTION-ID"))
     (if (and (= pclclass "Restriction" )(= pclstate "existing"))(setq newlayer "RESTRICTION-EXISTING" textlayer "TEXT-RESTRICTION-ID"))

     (if (and (= pclclass "Road" )(= pclstate "affected"))(setq newlayer "ROAD-AFFECTED" textlayer "TEXT-ROAD-ID"))
     (if (and (= pclclass "Road" )(= pclstate "extinguished"))(setq newlayer "ROAD-CANCELLED" textlayer "TEXT-ROAD-ID"))
     (if (and (= pclclass "Road" )(= pclstate "created"))(setq newlayer "ROAD-CREATED" textlayer "TEXT-ROAD-ID"))
     (if (and (= pclclass "Road" )(= pclstate "encroached"))(setq newlayer "ROAD-ENCROACHED" textlayer "TEXT-ROAD-ID"))
     (if (and (= pclclass "Road" )(= pclstate "existing"))(setq newlayer "ROAD-EXISTING" textlayer "TEXT-ROAD-ID"))

     (if (and (= pclclass "Stage Lot" )(= pclstate "affected"))(setq newlayer "STAGE-LOT-AFFECTED" textlayer "TEXT-STAGE-LOT-ID"))
     (if (and (= pclclass "Stage Lot" )(= pclstate "extinguished"))(setq newlayer "STAGE-LOT-CANCELLED" textlayer "TEXT-STAGE-LOT-ID"))
     (if (and (= pclclass "Stage Lot" )(= pclstate "created"))(setq newlayer "STAGE-LOT-CREATED" textlayer "TEXT-STAGE-LOT-ID"))
     (if (and (= pclclass "Stage Lot" )(= pclstate "encroached"))(setq newlayer "STAGE-LOT-ENCROACHED" textlayer "TEXT-STAGE-LOT-ID"))
     (if (and (= pclclass "Stage Lot" )(= pclstate "existing"))(setq newlayer "STAGE-LOT-EXISTING" textlayer "TEXT-STAGE-LOT-ID"))

     (if (= pclclass "Easement" )(setq newlayer "EASEMENT" textlayer "TEXT-EASEMENT-ID"))
     (if (= pclclass "Crown Land Service" )(setq newlayer "CROWN-LAND-SERVICE" textlayer "TEXT-CROWN-LAND-SERVICE-ID"))
     

       
     	(if (and (/= newlayer nil)(/= textlayer nil))
	  (progn
	(command "copy" en "" "0,0,0" "0,0,0")
	(SETQ SENT (ENTLAST))
	(SETQ SENTLIST (ENTGET SENT))
	(SETQ NEWSENTLIST (subst (cons 8 newlayer)(assoc 8 SENTLIST) SENTLIST ))
	(ENTMOD NEWSENTLIST)
	(command "-layer" "m" textlayer "")
	(COMMAND "TEXT" "J" "BC" lotccord (* TH 2) "90" pclname)
	))

    (if (= (substr pclname 1 3) "LGA")(setq newlayer "BOUNDARY-LGA"))
    (if (= (substr pclname 1 3) "PSH")(setq newlayer "BOUNDARY-PARISH"))
    (if (= (substr pclname 1 3) "TWN")(setq newlayer "BOUNDARY-TOWNSHIP"))

    (IF (OR (= (substr pclname 1 3) "LGA")(= (substr pclname 1 3) "PSH")(= (substr pclname 1 3) "TWN"))
      (progn
	(command "copy" en "" "0,0,0" "0,0,0")
	(SETQ SENT (ENTLAST))
	(SETQ SENTLIST (ENTGET SENT))
  ...
  (IF (/= (setq lots (ssget "_X" '((0 . "POINT") (8 . "Non SCFF PM")))) nil)(progn
			(setq count 0)
  (repeat (sslength lots)
     (SETQ EN (CDR(ASSOC -1 (ENTGET (SSNAME lots COUNT)))))
    (SETQ P1 (CDR(ASSOC 10 (ENTGET (SSNAME lots COUNT)))))
...
(setq wwpos (vl-string-position 34 xdatai (+ stringpos 5)))(setq oid (substr xdatai (+ stringpos 6) (-(- wwpos 1)(+ stringpos 4))))))
    ...
    (setq newlayer nil textlayer nil)
    (if (= (substr oid 1 1) "1")(setq newlayer "POINT-CONTROL-PCM" textlayer "TEXT-POINT-CONTROL-ID")(setq newlayer "POINT-CONTROL-PM" textlayer "TEXT-POINT-CONTROL-ID"))
	...
	(ENTMOD NEWSENTLIST)
	(command "-layer" "m" textlayer "")
	(COMMAND "TEXT" "J" "L" p1 (* TH 2) "90" oid)
	))
    (setq count (+ count 1))
    );repeat
));p&if PM's PCMS
